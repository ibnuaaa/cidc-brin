...
$('#modalDelete').on('show.bs.modal', function(e) {
    ...
    $('#deleteAction').click(function() {
        axios.delete('/receive_request/'+recordId).then((response) => {
...
function buatPenerimaan(purchase_order_id) {
    showLoading()
    axios.post('/receive_request', {
        purchase_order_id: purchase_order_id
    }).then((response) => {
        location.href = '/receive_request/edit_item/' + response.data.data.id
    }).catch((error) => {
        console.log(error)
    })
