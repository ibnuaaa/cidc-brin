<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        distributor.on('select2:select', function (e) {
            console.log('aaaa')
            updatePurchaseOrder(e.params.data.id)
        });
    ...
    function saveMaterial(material_id) {
        showLoading()
        axios.post('/purchase_order_item', {
    ...
    function updatePurchaseOrder(distributor_id) {
        showLoading()
        axios.put('/purchase_order/{{ $data['id'] }}', {
    ...
    function updateMaterial(e, id) {
        showLoading()
        axios.put('/purchase_order_item/' + id, {
            qty: $(e).val(),
        }).then((response) => {
            location.reload()
        }).catch((error) => {
    ...
    function deleteItem(id) {
        showLoading()
        axios.delete('/purchase_order_item/'+id).then((response) => {
            location.reload()
        }).catch((error) => {
            if (Boolean(error) && Boolean(error.response) && Boolean(error.response.data) && Boolean(error.response.data.exception) && Boolean(error.response.data.exception.message)) {
                Swal.fire({ title: 'Opps!', text: error.response.data.exception.message, type: 'error', confirmButtonText: 'Ok' })
